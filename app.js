...
/**
 * Нормализует название операции по ключевым словам
 * @param {string} userInput - введённое пользователем название
 * @param {Array} operationsList - список операций
...
/**
 * Проверяет URL на наличие токена после редиректа Google
...
/**
 * Инициализация: проверяет сохранённый токен или запускает локальный режим
 */
...
// ==================== 5-А. ОБЁРТКИ ДЛЯ ЛОКАЛЬНОГО РЕЖИМА ====================
/**
 * Безопасная запись в Google Sheets (если нет токена – сохраняет в очереди)
 */
...
/**
 * Безопасное добавление строки в Google Sheets
 */
...
/**
 * Модальное окно добавления/редактирования операции ТО
 */
...
/**
 * Модальное окно выбора автомобиля (Google Sheets ID)
 */
...
function openPartForm(part = null) {
    ...
    form.onsubmit = (e) => {
        ...
        if (isEdit) {
            ...
            safeAppendSheet('PartsCatalog!A:I', [row]).then(() => loadSheet()).catch(e => console.warn(e));
        }
...
/**
 * Модальное окно добавления/редактирования заправки
 */
...
/**
 * Модальное окно добавления/редактирования шин
 */
...
// ==================== 11. СОХРАНЕНИЕ ====================
/**
 * Добавление записи о выполненном ТО
 */
async function addServiceRecord(opId, date, mileage, motohours, partsCost, workCost, isDIY, notes, photoUrl) {
    ...
    if (isOnline && accessToken) {
        await safeWriteSheet(`Журнал ТО!C${op.rowIndex}:E${op.rowIndex}`, values);
    ...
    renderTOTable();
    localStorage.setItem(CACHE_KEY, JSON.stringify({ operations, settings, parts, fuelLog, tireLog, workCosts }));
    await loadSheet(); // для обновления UI
}

/**
 * Сохранение операции ТО (создание/редактирование)
 */
async function saveOperation(data) {
    ...
    if (id && !isNaN(rowIndex) && rowIndex >= 2) {
        ...
        if (accessToken) await safeAppendSheet('Журнал ТО!A:H', [rowData]);
        else {
            const newId = operations.length + 2;
            operations.push({
                id: newId, rowIndex: newId, category, name, intervalKm: parseInt(km)||0, intervalMonths: parseInt(months)||0, intervalMotohours: moto?parseInt(moto):null,
...
/**
 * Автоматическое добавление зависимых операций (например, фильтр при замене масла)
 */
