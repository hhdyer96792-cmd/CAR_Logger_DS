    ...
    const modal = createModal('➕ Выполнить ТО', `
        <form id="service-form" enctype="multipart/form-data">
            ...
            ` : `
              ...
              <label class="checkbox-label"><input type="checkbox" name="isDIY" value="true">Сделал сам</label>
                `}
...
function openTireModal(record=null) {
    ...
    const modal = createModal(isEdit?'✏️ Редактировать запись шин':'🛞 Сменить резину', `
        <form id="tire-form">
            ...
            <div id="mountFields" style="display:${isNewSet?'none':'block'};"><label>Текущий пробег (км)</label><input type="number" name="currentMileage" value="${isNewSet?0:settings.currentMileage}" required><label>Стоимость шиномонтажа (₽)</label><label class="checkbox-label"><input type="checkbox" name="isDIY" value="true">Сделал сам</label>
...
function generateShoppingList(opId) {
    ...
    if (!items.length) { showToast('Нет запчастей для этой операции', 'warning'); return; }
    let listHtml = `<div style="max-height: 60vh; overflow-y: auto;"><strong>${op.name}</strong><ul>`;
    items.forEach(p => {
        const stock = p.inStock || 0;
        const location = p.location ? ` (${p.location})` : '';
        if (stock > 0) {
            listHtml += `<li>${p.oem || p.analog} ${p.price ? p.price+'₽' : ''} — ✅ есть на складе: ${stock} шт.${location}</li>`;
        } else {
            listHtml += `<li>${p.oem || p.analog} ${p.price ? p.price+'₽' : ''} — ❌ нужно купить</li>`;
        }
    });
    listHtml += '</ul></div>';
    const modal = createModal('Список запчастей', listHtml);
...
function renderMaintenancePlan() {
    ...
    let html = '<table class="plan-table"><thead><tr><th>Операция</th><th>Категория</th><th>Плановая дата</th><th>Плановый пробег</th><th></th></tr></thead><tbody>';
    plan.forEach(op => {
